...
Boot Record placed at sector 0
FAT32 File System starts at sector 32
Extended Boot Record (EBR) starts at sector 36
Root Directory starts at sector 44
Sector 510 and 511 are reserved for boot signature 0x55AA

References:
- https://wiki.osdev.org/FAT
- https://wiki.osdev.org/FAT32
- https://www.osdever.net/bkerndev/Docs/fat32.html
...
#define ATTR_DIRECTORY 0x10     // This is for directory
#define ATTR_ARCHIVE   0x20     // This is for file
...
uint32_t fat32_cluster_to_sector(uint32_t cluster) {
    // Cluster 0 and Cluster 1 are reserved by the FAT32 specification.
    // The actual data area (cluster heap) starts from Cluster 2
...
uint32_t fat32_find_free_cluster() {
    uint8_t sector[512];
    for (uint32_t i = 2; i < 0x0FFFFFF6; i++) { // 0x00000002 - 0x0FFFFFEF usable cluster
        uint32_t fat_offset = i * 4;    // 4 bytes per cluster in fat32
...
bool fat32_create_file(const char* filename) {
    ...
    if(!ahci_read(fat32_port, root_sector, 0, 1, (uint16_t*)buffer)){
        return false;
    }
    ...
    for (int i = 0; i < 512 / sizeof(DIR_ENTRY); i++) {
        if (entries[i].name[0] == 0x00 || entries[i].name[0] == 0xE5) {
            ...
            entries[i].attr = ATTR_ARCHIVE;         // This is file
            ...
            if(!ahci_write(fat32_port, root_sector, 0, 1, (uint16_t*)buffer)){
                return false;
            }
            return true;
        }
...
bool fat32_read_file(const char* filename, uint8_t* buffer, uint32_t max_size) {
    ...
    if(!ahci_read(fat32_port, root_sector, 0, 1, (uint16_t*)sector)){
        return false;
    }
        ...
        if (strncmp(entries[i].name, filename, 11) == 0) {
            ...
            while (cluster < 0x0FFFFFF8 && bytes_read < size) {
                ...
                for (uint32_t j = 0; j < fat32_info.sectors_per_cluster; j++) {
                    ...
                    if(!ahci_read(fat32_port, sector_num + j, 0, 1, (uint16_t*)temp)){
                        return false;
                    }
...
bool fat32_write_file(const char* filename, const uint8_t* data, uint32_t size) {
    ...
    for (int i = 0; i < 512 / sizeof(DIR_ENTRY); i++) {
        if (strncmp(entries[i].name, filename, 11) == 0) {
            ...
            if(!ahci_write(fat32_port, root_sector, 0, 1, (uint16_t*)sector)){
                return false;
            }
...
uint32_t fat32_next_cluster(uint32_t cluster) {
    ...
    if(!ahci_read(fat32_port, fat_sector, 0, 1, (uint16_t*)sector)){
        return false;
    }
...
bool fat32_create_directory(const char* name) {
    ...
    // Write back with updated cluster entry
...
bool fat32_find_free_entry(uint32_t cluster, DIR_ENTRY* entry) {
    ...
    if(!ahci_read(fat32_port, sector, 0, 1, (uint16_t*)buffer)){
        return false;
    }
...
bool fat32_write_directory_entry(uint32_t cluster, DIR_ENTRY* entry) {
    ...
    for (int i = 0; i < 512 / sizeof(DIR_ENTRY); i++) {
        if (entries[i].name[0] == 0x00 || entries[i].name[0] == 0xE5) {
            ...
            if(!ahci_write(fat32_port, sector, 0, 1, (uint16_t*)buffer)){
                return false;
            }
