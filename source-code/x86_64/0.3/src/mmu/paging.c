...
void init_paging(){
    ...
    pml4_table[511].p = 1;  // Using last PML4 entry to map higher-half
    pml4_table[511].rw = 1;
    ...
    pt_table[0].p = 1;
    pt_table[0].rw = 1;
    pt_table[0].page_addr = kernel_phys_base >> 12; // Physical address of kernel
...
uint64_t get_phys_addr(uint64_t vir_addr, pml4_t* pml4_base){
    ...
    pdpt_t * pdpr_base = (pdpt_t *) (pml4_entry.tab_addr << 12);  // To get the base address
    ...
    // Step 4 : Traverse PD
    pd_t* pd_base = (pd_t*)(pdpt_entry.tab_addr << 12);
    ...
    // Step 5: Traverse PT
    pt_t* pt_base = (pt_t*)(pd_entry.tab_addr << 12);
...
uint64_t phys_to_virt(uint64_t phys_addr, pml4_t* pml4_base) {
    ...
    for (uint64_t pml4_index = 0; pml4_index < ENTRY_COUNT; pml4_index++) {
        ...
        pdpt_t* pdpt_base = (pdpt_t*)(pml4_entry.tab_addr << 12);
