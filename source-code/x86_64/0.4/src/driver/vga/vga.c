...
uint64_t text_color = (uint64_t) DEFAULT_TEXT_COLOR;
uint64_t back_color = (uint64_t) DEFAULT_BACK_COLOR;
...
void scroll_up() {
    ...
    memmove(fb_ptr, fb_ptr + move_up_pixels, (SCREEN_HEIGHT - line_height) * SCREEN_WIDTH * sizeof(uint64_t));
...
void draw_gradient( uint64_t start_color, uint64_t end_color) {
    for (size_t y = 0; y < SCREEN_HEIGHT; y++) {
        uint64_t color = start_color + (y * ((end_color - start_color) / SCREEN_HEIGHT));
        for (size_t x = 0; x < SCREEN_WIDTH; x++) {
            set_pixel( x, y, color);
        }
    }
