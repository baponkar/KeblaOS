...
void putchar(unsigned char c){
    if (c == '\t'){
        ...
        cur_pos_x = (cur_pos_x + 4 * DEFAULT_FONT_WIDTH) & ~( 4 * DEFAULT_FONT_WIDTH - 1);
    }else if( c == '\r'){
...
void update_cur_pos(){
    if(cur_pos_x > SCREEN_WIDTH - 1){
        cur_pos_y += line_height;
        cur_pos_x = 0;
    }

    if(cur_pos_y > SCREEN_HEIGHT - 1){
        scroll_up();
        cur_pos_y = SCREEN_HEIGHT - 1 - line_height;
        cur_pos_x = 0;
    }
}
