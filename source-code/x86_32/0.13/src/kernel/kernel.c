...
void kmain (uint32_t magic, uint32_t addr)
{
    ...
    init_system();  // Initialise System functions

    print_linker_pointer_info();    // Printing _start and _end address
    print_mutiboot_info(magic, addr);   // Printing Different Multiboot Information
    get_cpu_info(); // Print CPU Information

    uint32_t a = kmalloc(8);
    uint32_t b = kmalloc(8);
    uint32_t c = kmalloc(8);
    printf("Value: a=%x,b=%x,c=%x\n",a,b,c);
    printf("Pointer Loc: a=%x,b=%x,c=%x\n", &a, &b, &c);
