...
bool uefi_install(int iso_disk_no, int boot_disk_no, uint64_t boot_disk_sectors){
    ...
    printf("[INSTALLER] Successfully Mount FAT32 FS\n");
...
{
    if(!fat32_mount( start_lba)) return false;

    FAT32_STAT stat;

    // Check present of files
    if(!f_stat("/boot/kernel.bin", &stat)) return false;

    if (!f_stat("/efi/boot/limine.conf", &stat)) return false;

    if (!f_stat("/boot/user_main.elf", &stat)) return false;

    if (!f_stat("/efi/boot/bootx64.efi", &stat)) return false;
