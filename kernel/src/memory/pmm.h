...
#define BIT_NO_TO_ADDR(bit_no) ((uint64_t)USABLE_START_PHYS_MEM + (uint64_t)(bit_no * FRAME_SIZE))

// Converting physical address to bit number
#define PHYS_ADDR_TO_BIT_NO(phy_addr)(((uint64_t)phy_addr <= (uint64_t)USABLE_START_PHYS_MEM) ? 0 : (((uint64_t)phy_addr - (uint64_t)USABLE_START_PHYS_MEM) / (uint64_t)FRAME_SIZE))
