...
uint64_t alloc_stack() {
    void* stack = kheap_alloc(16 * 4096); // Ensure this function returns valid memory
    if (!stack) {
...
void delete_process(process_t *process) {
    ...
    kheap_free((void*)(process->regs->iret_rsp - (16 * 4096)), sizeof(uint64_t)); // Free the stack
    kheap_free((uint64_t *)process, sizeof(process_t)); // Free the process structure
}
