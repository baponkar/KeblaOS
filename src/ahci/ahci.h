/*
D2H : device to host
Device : HDD, Optical HDD, SSD
FIS : Frame Information Structure - A FIS is a packet or frame of information that is
transferred between the host and device. Refer to the Serial ATA 1.0a specification for more information.
HBA : Host Bus Adapter - A host bus adapter refers to the silicon that implements the
AHCI specification to communicate between system memory and Serial ATA devices.
Port : 
PRD : PRD is an acronym for “physical region descriptor”. PRD entries describe the physical location and
...
typedef struct {
    uint8_t fis_type;   // FIS Type (0xA1 for Device Bits FIS)
