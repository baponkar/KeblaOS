...
void target_cpu_task(struct limine_smp_info *smp_info) {
    ...
    uint32_t core_id = smp_info->lapic_id; // Or map LAPIC ID to core ID
    // Initialize GDT, IDT, and TSS for this core
    core_init(core_id);
    // init_core_cpu_interrupt(core_id);
    printf("CPU %d initialized and running.\n", core_id);

    // Configure APIC timer for this core
    // init_apic_timer(10); // 10 ms interval
    ...
    // Enter scheduler loop or wait for tasks
    while (1) {
        asm volatile("hlt");
    }
}
...
void switch_to_core(uint32_t target_lapic_id) {
    ...
    for (size_t i = 0; i < smp_request.response->cpu_count; i++) {
        ...
        if (smp_info->lapic_id == target_lapic_id) {
            ...
            lapic_send_ipi(target_lapic_id, 0x20); // 0x20 is the vector
            break;
        }
    }
...
void start_secondary_cores1() {
    int num_cores = getLogicalProcessorCount();
    for (int core = 1; core < num_cores; core++) {
        ...
        // Allow time for the core to initialize
        printf("Started CPU %d (LAPIC ID %d)\n", core, smp_info->lapic_id);
    }
}
