...
uint8_t user_program[] = {
    ...
    // 0xf7, 0xf1,                      // div rcx
    // 0xb8, 0x78, 0x56, 0x34, 0x12,    // mov $0x12345678, rax
    0xb8, 0xac,                     // Store 0xac System Call in registers a
    // 0xcd, 0xac,                     // int $0xac : Read System call interrupt
    0xcd, 0xad,                     // int $0xad : Print System Call Interrupt
    0xcd, 0xae,                     // int $0xae : Exit System Call Interrupt
    0xeb, 0xfe                      // jmp 0x9   : 
};



uint64_t create_user_function(void * user_program) {
    void *user_code_entry = (void *) uheap_alloc(0x1000);

    // Copy user_program array contents into user_code_entry location
    memcpy(user_code_entry, user_program, sizeof(user_program));

    return (uint64_t)user_code_entry;         // Return the user-accessible function pointer
}


void init_user_mode(){
    uint64_t code_addr = (uint64_t) create_user_function((void *)&user_program);
    ...
    printf("[Info] Starting Switching to the user mode: code addr.- %x, stack addr.- %x\n", code_addr, stack_top_addr);
